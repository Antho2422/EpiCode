...
if isempty(cfg.circus.paramfile)
    ...
    fname_params_default    = fullfile(p, 'projects', 'epinjector', 'src', 'templates', 'SpykingCircus.params')
else
...
for ipart = cfg.circus.part_list
    ...
    if isempty(cfg.circus.channelname)
        ...
        temp = dir(fullfile(cfg.datasavedir, '*.dead'));
        if isempty(temp)
            continue
        end
        if temp(1).bytes == 0
        ...
        fname_params    = fullfile(cfg.datasavedir, filename);
        ...
        writeProbeFile(nb_channels, fullfile(cfg.datasavedir, fname_prb));
        ...
        for chandir = unique(cfg.circus.channelname)
            ...
            temp        = dir(fullfile(cfg.datasavedir, string(chandir), '*.dead'));
            if isempty(temp)
                continue
            end
            if temp(1).bytes == 0
            ...
            filename        = 'SpykingCircus.params';            
            fname_params    = char(fullfile(cfg.datasavedir, string(chandir), filename));
            fname_prb       = ['Adtech_', num2str(nb_channels), 'chan.prb'];
            status          = ini.WriteFile(fname_params);
            if status == false
                error('Couldn''t write file %s', fname_params);
            ...
            writeProbeFile(nb_channels, fullfile(cfg.datasavedir, string(chandir), fname_prb));
