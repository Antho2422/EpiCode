...
for ipart = 1 : size(cfg.directorylist, 2)
    ...
    if ~exist(fullfile(cfg.datasavedir), 'dir')
        sprintf('Creating directory %s', fullfile(cfg.datasavedir));
        mkdir(fullfile(cfg.datasavedir));
    end
    ...
    if isempty(cfg.circus.channelname)
        ...
        dlmwrite(fullfile(cfg.datasavedir, filename), deadfile_ms, 'delimiter', '	', 'precision', '%.4f');
        
        filename = sprintf('SpykingCircus_artefacts_samples%s.dead', suffix);
        fprintf('Writing artefacts for Spyking-Circus to: %s\n', filename);
        dlmwrite(fullfile(cfg.datasavedir, filename), deadfile_samples, 'delimiter', '	', 'precision', '%.4f');    
    else
        for chandir = unique(cfg.circus.channelname)
            
            % if it doesnt exist, create directory
            if ~exist(fullfile(cfg.datasavedir, string(chandir)), 'dir')
                sprintf('Creating directory %s', fullfile(cfg.datasavedir, string(chandir)));
                mkdir(fullfile(cfg.datasavedir, string(chandir)));
            end

            filename = sprintf('SpykingCircus_artefacts_ms%s.dead', suffix);
            fprintf('Writing artefacts for Spyking-Circus to: %s\n', filename);
            dlmwrite(fullfile(cfg.datasavedir, string(chandir), filename), deadfile_ms, 'delimiter', '	', 'precision', '%.4f');
            
            filename = sprintf('SpykingCircus_artefacts_samples%s.dead', suffix);
            fprintf('Writing artefacts for Spyking-Circus to: %s\n', filename);
            dlmwrite(fullfile(cfg.datasavedir, string(chandir), filename), deadfile_samples, 'delimiter', '	', 'precision', '%.4f');
        end
    end
