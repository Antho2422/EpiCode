...
for ipart = 1 : size(cfg.directorylist, 2)
    ...
    if isempty(cfg.circus.channelname)
        ...
        fname       = fullfile(cfg.datasavedir, chandir, 'filelist.txt');
       
        % if it doesnt exist, create directory
        if ~exist(fullfile(cfg.datasavedir), 'dir')
            fprintf('Creating directory %s', fullfile(cfg.datasavedir));
        ...
        if ~exist(fullfile(cfg.datasavedir, chandir), 'dir')
            fprintf('Creating directory %s', fullfile(cfg.datasavedir, chandir));
            mkdir(fullfile(cfg.datasavedir, chandir));
        end
        fid = fopen(fname, 'w');
